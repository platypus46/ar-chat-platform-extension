...
const component = AFRAME.registerComponent("finger-cursor", {
  schema: {
    lineColor: {
      default: "transparent"   //이 부분 임의로 바꿈 ㅠㅠ
    },
    lineLength: {
      default: "5"
    },
  ...
  init: function () {
    this.el.setAttribute("line", {
      "color": "transparent",   //이 부분도 임의로 바꿈
      "start": "0 0 0",
      "opacity": 0.25,
      "end": "0 0 -2"
    });
...
const component = AFRAME.registerComponent("hand-teleport", {
  ...
  tick: function () {
    ...
    return function () {
      ...
      if (!this.joints) return; // teleport origin

      let wrist = this.joints.Wrist; // teleport when the index is perp. do the wrist
